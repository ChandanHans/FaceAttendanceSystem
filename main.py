...
class AddStudentFrame(CTkFrame):
    ...
    def create_widgets(self):
        ...
        self.submit_button = CTkButton(
            self.ask_data_frame, text="Add Student", command=self.add_student
        )
...
class ShowCameraFrame(CTkFrame):
    def __init__(self, parent: AddStudentFrame, camera_index, **kwargs):
        ...
        self.button_frame = CTkLabel(self.image_label, text="", height=30)
    ...
    def update_frame(self):
        if not self.running:
            return  # Stop updating if the camera has been stopped

        ret, frame = self.cap.read()
        if not ret:
            return  # If frame is not read correctly
