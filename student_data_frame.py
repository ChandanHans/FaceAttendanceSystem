...
class StudentDataFrame(CTkFrame):
    def __init__(self, parent, db : Database, **kwargs):
        ...
        self.download_logo = CTkImage(Image.open(resource_path("src/download.png")), size=(30, 30))
    ...
    def create_filter_frame(self):
        ...
        self.filter_frame.pack(side="top", fill="x", padx=6, pady=(6, 1),ipadx=10,ipady=10)
        ...
        self.max_date_entry.bind("<Button-1>", lambda event : self.open_date_picker(event, self.max_date))
        ...
        self.min_date_entry.bind("<Button-1>", lambda event : self.open_date_picker(event, self.min_date))
    ...
    def show_table(self,*args):
        ...
        if self.dates:
            ...
            yscroll = tk.Scrollbar(self.tree, orient="vertical", command=self.tree.yview, width=15)
            yscroll.pack(side=tk.RIGHT, fill=tk.Y)
            
            xscroll = tk.Scrollbar(self.tree, orient="horizontal", command=self.tree.xview, width=15)
            xscroll.pack(side=tk.BOTTOM, fill=tk.X)
            
            self.tree.configure(yscrollcommand = yscroll.set,xscrollcommand= xscroll.set)
            
            # Define columns
            self.tree["columns"] = ["Roll","Name"] + self.dates
            ...
            for i in self.dates:
                ...
                self.tree.heading(i, text=datetime.strptime(i,"%Y-%m-%d").strftime("%d/%m/%Y"), anchor=tk.W)
...
"""
    ...
    def open_date_picker(self, event, input_field_text : StringVar):
        ...
        def on_date_select(event):
            ...
            self.set_date_list()
            self.show_table()
        cal = Calendar(self, selectmode='day', mindate=self.oldest_date, maxdate=self.recent_date)
    ...
    def set_default_date(self):
        ...
        self.min_date.set(max(first_day_of_month,self.oldest_date).strftime('%Y-%m-%d'))
